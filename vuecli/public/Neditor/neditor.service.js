...
UE.Editor.prototype.getActionUrl = function(action) {
    // console.log("getActionUrl:----",action,window.location.hostname)
    /* 按config中的xxxActionName返回对应的接口地址 */
    if (action == 'uploadfile' || action == 'uploadimage' || action == 'uploadscrawl' || action == 'uploadvideo') {
        // let res_url='https://keyportaldev.krclouds.com/krportalapi/v1/FileUpload/Upload/'+window.localStorage.getItem("u_tid")+"/4";
        if (window.location.hostname == "localhost") {
            let res_url = 'https://keyportaldev.krclouds.com/krportalapidev/v1/FileUpload/Upload/' + window.localStorage.getItem("u_tid") + "/" + window.localStorage.getItem("u_sid") + "/4";
            res_url += "?access_token=" + window.localStorage.getItem("u_user_token");
            return res_url
        } else {
            let res_url = window.location.href.substring(0, window.location.href.indexOf("/", window.location.protocol.length + 2)) + '/krportalapi/v1/FileUpload/Upload/' + window.localStorage.getItem("u_tid") + "/" + window.localStorage.getItem("u_sid") + "/4";
            res_url += "?access_token=" + window.localStorage.getItem("u_user_token");
            return res_url
        }
    } else {
...
window.UEDITOR_CONFIG['imageUploadService'] = function(context, editor) {
    return {
        ...
        setFormData: function(object, data, headers) {
            headers["token"] = window.localStorage.getItem("u_user_token");
            return data;
        },
        /**
         * 触发startUpload事件时执行
    ...
    return scrawlUploadService = {
        ...
        uploadScraw: function(file, base64, success, fail) {
            ...
            $.ajax({
                ...
                res.responseSuccess = res.code == 2000;

                /* 指定上传接口返回的response中涂鸦图片路径的字段，默认为 url 
                 * 如果涂鸦图片路径字段不是res的属性，可以写成 对象.属性 的方式，例如：data.url
                 */
                res.scrawlSrcField = 'result.Url';
